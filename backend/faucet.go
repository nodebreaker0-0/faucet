...
func main() {
	err := godotenv.Load(".env_crescent")
	if err != nil {
		log.Fatal("Error loading .env file")
	}

	chain = getEnv("FAUCET_CHAIN")
	amountCre = getEnv("FAUCET_AMOUNT_Cre")
...
func getCoinsHandler(w http.ResponseWriter, request *http.Request) {
	...
	sendFaucet := fmt.Sprintf("crescentd tx bank send %v %v %v --chain-id=%v -y --home /root/.crescent --node %v --keyring-backend test",
		key, address, amountCre, chain, node)
